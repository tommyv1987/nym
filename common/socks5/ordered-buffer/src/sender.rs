...
impl OrderedMessageSender {
    ...
    /// Turn raw bytes in an OrderedMessage containing the original bytes
    /// and a sequence number;
    pub fn to_message(&mut self, input: Vec<u8>) -> OrderedMessage {
...
mod ordered_message_sender {
    ...
    mod sequence_index_numbers {
        ...
        fn increase_as_messages_are_sent() {
            ...
            let first_message = sender.to_message(first_bytes);

            assert_eq!(first_message.index, 0);

            let second_message = sender.to_message(second_bytes);
